...
class AttendanceDao extends BaseDao
{
    ...
    /**
     * Get daily attendance totals per employee grouped by date.
     * Returns rows with: empNumber, firstName, lastName, middleName, employeeId, terminationId, workDate, total (seconds).
     *
     ...
     * @return array
     */
    public function getEmployeeDailyHoursCompliance(DateTime $fromDate, DateTime $toDate, array $empNumbers): array
    {
        $q = $this->createQueryBuilder(Employee::class, 'employee');
        $q->leftJoin('employee.attendanceRecords', 'attendanceRecord', Expr\Join::WITH, $q->expr()->andX(
        ...
        $q->select(
            ...
            'employee.employeeId AS employeeId',
            'IDENTITY(employee.employeeTerminationRecord) AS terminationId',
            "SUBSTRING(attendanceRecord.punchOutUserTime, 1, 10) AS workDate",
            "SUM(TIME_DIFF(COALESCE(attendanceRecord.punchOutUtcTime, 0), COALESCE(attendanceRecord.punchInUtcTime, 0),'second')) AS total"
        );
        ...
        // Only include records that have been punched out
        $q->andWhere($q->expr()->isNotNull('attendanceRecord.punchOutUtcTime'));

        $q->groupBy('employee.empNumber');
        $q->addGroupBy('workDate');
        $q->addOrderBy('employee.lastName', ListSorter::ASCENDING);
        $q->addOrderBy('workDate', ListSorter::ASCENDING);
